...
function queuesInfoEmbed(queues: GuildQueueStatisticsMetadata[]) {
	...
	for (const queue of queues) {
		...
		queueInfo.addFields(
			{ name: "Event Loop Lag (ms)", value: queue.latency.eventLoop.toPrecision(2).toString(), inline: true },
			{ name: "Tracks Count", value: queue.tracksCount.toString(), inline: true },
			{ name: "History Size", value: queue.historySize.toString(), inline: true },
			{ name: "Rss (MB)", value: (queue.memoryUsage.rss / (1024 * 1024)).toFixed(2).toString(), inline: true },
			{ name: "Heap Total (MB)", value: (queue.memoryUsage.heapTotal / (1024 * 1024)).toFixed(2).toString(), inline: true },
			{ name: "Heap Used (MB)", value: (queue.memoryUsage.heapUsed / (1024 * 1024)).toFixed(2).toString(), inline: true },
		)
